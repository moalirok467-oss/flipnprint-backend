...
def fetch_product():
    ...
    try:
        ...
        sys.stdout.write(f"Requesting: {scraperapi_url} with key: {SCRAPERAPI_KEY[:10]}...\n")
        sys.stdout.flush()
        ...
        if response.status_code != 200:
            return jsonify({"error": f"ScraperAPI error: {response.status_code} - {response.text[:200]}"}), 500
        ...
        # If meta tags didn't work, try scraping from page
        if not title:
