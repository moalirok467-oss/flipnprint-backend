...
def early_access():
    ...
    # In-memory storage (fine for launch; move to Supabase later)
    if not hasattr(early_access, "emails"):
        early_access.emails = set()
    if email in early_access.emails:
        return jsonify({"message": "Already on the list"}), 200
    early_access.emails.add(email)
    print(f"EARLY_ACCESS: {email} joined (total: {len(early_access.emails)})", flush=True)
