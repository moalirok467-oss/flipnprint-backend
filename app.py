...
def generate_drops_background():
    ...
    try:
        ...
        # 2. Process candidates in parallel waves of 3 until we have enough
        candidates = [a for a in asins[:30] if a not in done_asins]
        for i in range(0, len(candidates), 3):
            with DROPS_LOCK:
                if len(DROPS["items"]) >= DROPS_COUNT:
                    break
            wave = candidates[i:i+3]
            with ThreadPoolExecutor(max_workers=3) as pool:
                futures = [pool.submit(process_asin, a) for a in wave]
