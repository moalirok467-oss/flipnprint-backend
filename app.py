...
# Early access list storage
EARLY_ACCESS_LIST = {}  # email -> timestamp
...
def early_access():
    ...
    print(f"EARLY_ACCESS: {email} joined (total: {len(EARLY_ACCESS_LIST)})", flush=True)
    return jsonify({"message": "Added to early access list"}), 201

@app.route("/admin/early-access", methods=["GET"])
def admin_early_access():
    email = verify_google(request)
    if not email or email != OWNER_EMAIL:
        return jsonify({"error": "Unauthorized"}), 403
    emails = sorted(EARLY_ACCESS_LIST.items(), key=lambda x: x[1], reverse=True)
    return jsonify({"total": len(EARLY_ACCESS_LIST), "emails": emails})
