...
# ===== DAILY DROPS ENGINE =====
DROPS = {"date": None, "items": [], "generating": False, "error": None}
...
def generate_drops_background():
    ...
    try:
        ...
        for asin in asins[:DROPS_COUNT * 2]:  # extra candidates in case some fail
            with DROPS_LOCK:
...
def drops():
    ...
    today = time.strftime("%Y-%m-%d")
    with DROPS_LOCK:
        if DROPS["date"] != today and not DROPS["generating"]:
            DROPS["date"] = today
            DROPS["items"] = []
            DROPS["error"] = None
